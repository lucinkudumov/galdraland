...
//            var populateQuery = [{path:'user', model:'User'}, {path:'adventure', model: 'Adventure', populate:{path: 'owner', model:'User'}}];
            faModel.find({"createdAt": {$gt: date}}).populate([{path:'user', model:'User'}, {path:'adventure', model: 'Adventure'}]).deepPopulate({path: 'adventure.owner', model:'User'}).exec(function (err, fadventures) {
                if (err) {
                    console.log(err);
